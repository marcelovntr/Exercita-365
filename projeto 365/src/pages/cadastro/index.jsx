...
function CadastroLocalExercicio() {
  ...
  const { listalocais, dadosLocais, editarLocais, lerLocaisByIdAsync, lerLocaisById, mostrarCadLocal, 
    setCadLocal, mostrarEdicaoLocal, setMostrarEdicaoLocal, cadastrarLocais } = useContext(UsuariosContext);
...
console.log('depois de editar', formValue);
  ...
  useEffect(() => {
    if (!!id) {
      lerLocaisByIdAsync(id).then(dados => {
      ...
      {mostrarCadLocal && (
        <>
          ...
          <form className={styles.formlocais} onSubmit={handleSubmit(cadOnSubmit)}>
            ...
            <input type="hidden"
              value={localStorage.getItem('idCadastrante')}
              //FAZER RECEBER UM VALUE do localStorage
              {...register('idCadastrante')}
            />
